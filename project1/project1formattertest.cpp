...
bool SpecialFind(string text_str, string pattern_str)
{
    ...
    for (int i = 0; i < (int)text_str.size(); i++) {
        for (int j = 0; j < (int)pattern_str.size(); j++) {
            ...
            // Boolean for checking digit and # match
...
{
    // Finds match between text and pattern strings, formats for case
    // insensitivity, calls additional functions
...
int ImprovedMatches(const string& text_str, const string& pattern_str,
    const bool* is_case_insensitive)
{
    // iteration of Improved Contains on successively shrinking strs, count number
    // of matches
    int pattern_cnt = 0;
    string check_str = text_str;
    for (int i = 0; i < (int)text_str.size(); i++) {
...
void ImprovedReplace(string& text_str, const string& pattern_str,
    ...
    const bool* is_case_insensitive)
{
    // see hw 7, check if ImprovedContains, ImprovedFindFirstOf repeatedly, shrink
    // input str each time, build new output with replacement
