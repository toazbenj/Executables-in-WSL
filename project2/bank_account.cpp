...
vector<string> split(const string& input_str,
                     const char& delimeter_chr = '\n') {
  ...
  while (getline(ss, element, delimeter_chr)) out_vec.push_back(element);
...
void string_date_to_int_ptrs(const string& date_str, int* year_ptr,
  ...
  vector<int> out_vec;
  transform(split_vec.begin(), split_vec.end(), back_inserter(out_vec),
            [](auto p) { return stoi(p); });
...
int number_of_first_of_months(const string& date_begin_str,
  ...
  if (year1_int == year2_int)
    ...
    firsts_int =
        12 * (year2_int - year1_int - 1) + (12 - month1_int) + month2_int;
...
double interest_earned(double& balance_dub, const double& apr_dub,
  ...
  if (balance_dub > 0) {
    for (int i = 0; i < periods_int; i++) {
      ...
      deposit(balance_proxy_dub,
              interest_for_month(balance_proxy_dub, apr_dub));
    }
  }
...
void interest_output(const string& date_str, string& prev_date_str,
                     ...
                     stringstream& ss) {
  // Calculates interest for given period, formats output text, deposites
  // interest

  if (prev_date_str != "") {
    int interest_periods_int =
        number_of_first_of_months(prev_date_str, date_str);
    double interest_dub =
        interest_earned(balance_dub, apr_dub, prev_date_str, date_str);

    if (interest_periods_int != 0) {
      ss << "Since " << prev_date_str << ", interest has accrued "
         << interest_periods_int << " times." << endl
         << "$" << interest_dub << " interest has been earned." << endl;
...
void activity_output(const string& command_str, string& amount_str,
  ...
  if (command_str == "Deposit")
    ...
    bool is_successful =
        withdraw(balance_dub, stod(amount_str.substr(1, amount_str.size())));
